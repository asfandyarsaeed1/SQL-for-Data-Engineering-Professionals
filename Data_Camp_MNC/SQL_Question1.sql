...
SOLUTION::
select request_date,
ROUND(ABS(sum(distance_to_travel)/sum(monetary_cost) 
          - avg(sum(distance_to_travel)/sum(monetary_cost)) over(partition by extract(month from request_date)))::decimal,2) as ds
